...
// Constants times :
static CGFloat const kHooDatePickerAnimationDuration = 0.05f;
...
#define KHOODataPickerSureTxtColor [UIColor colorWithRed:0.0/255.0 green:0.0/255.0 blue:0.0/255.0 alpha:1.0]
#define KHOODataPickerCancelTxtColor [UIColor colorWithRed:0.0/255.0 green:0.0/255.0 blue:0.0/255.0 alpha:1.0]
...
typedef NS_ENUM(NSInteger,ScrollViewTagValue) {
    ...
    ScrollViewTagValue_WEEKS = 8
};

@interface HooDatePicker ()<UIScrollViewDelegate> {
    // Lines :
    UIView *_lineDaysTop, *_lineDaysBottom, *_lineMonthsTop, *_lineMonthsBottom, *_lineYearsTop,*_lineYearsBottom, *_lineDatesTop, *_lineDatesBottom, *_lineHoursTop, *_lineHoursBottom, *_lineMinutesTop, *_lineMinutesBottom, *_lineSecondsTop, *_lineSecondsBottom, *_lineWeeksTop, *_lineWeeksBottom;
    
    // Labels :
    NSMutableArray *_labelsDays, *_labelsMonths, *_labelsYears, *_labelsDates, *_labelsHours, *_labelsMinutes, *_labelsSeconds,
    *_labelWeeks;
    
    // Date and time and week selected :
    NSInteger _selectedDay,  _selectedMonth, _selectedYear,  _selectedDate, _selectedHour, _selectedMinute,  _selectedSecond,
    _selectdWeek, _maxWeekIndex;
    ...
    UIButton *dateModel;
    UIButton *timeModel;
    NSInteger initDateType;
    
}

// Data of years, months, days, dates, hours, minutes, seconds
@property (nonatomic, strong) NSMutableArray *years, *months, *days, *dates, *hours, *minutes, *seconds,*weeks;

// ScrollView for Years, Months, days ,Dates ,Hours ,Minute ,Seconds
@property (nonatomic, strong) UIScrollView *scrollViewYears, *scrollViewMonths, *scrollViewDays, *scrollViewDates, *scrollViewHours, *scrollViewMinutes,*scrollViewSeconds, *scrollViewWeeks;
...
@property(nonatomic, strong)NSString *sureTxt;

@property(nonatomic, strong)NSString *cancelTxt;

@property (nonatomic, strong) UIColor *sureTxtColor;

@property (nonatomic, strong) UIColor *cancelTxtColor;
...
- (void)setupControl {
    ...
    if (self.datePickerMode == HooDatePickerModeTime) {
        if (_showSecond) {
            ...
            [self buildSelectorHoursOffsetX:0.0 andWidth:self.frame.size.width * 0.5];
            [self buildSelectorMinutesOffsetX:(_scrollViewHours.frame.size.width + kHooDatePickerScrollViewLeftMargin) andWidth:self.frame.size.width * 0.5];
        }
    ...
    if (self.datePickerMode == HooDatePickerModeWeek) {
        _weeks = [self getWeeks];
        [self buildSelectorWeeksOffsetX:0.0 andWidth:self.frame.size.width];
    }
...
#pragma mark - Build Selector Weeks

- (void)buildSelectorWeeksOffsetX:(CGFloat)x andWidth:(CGFloat)width {
    
    _scrollViewWeeks = [[UIScrollView alloc] initWithFrame:CGRectMake(x, kHooDatePickerHeaderHeight + kHooDatePickerHeaderBottomMargin, width, self.frame.size.height - kHooDatePickerHeaderHeight - kHooDatePickerHeaderBottomMargin)];
    _scrollViewWeeks.tag = ScrollViewTagValue_WEEKS;
    _scrollViewWeeks.delegate = self;
    _scrollViewWeeks.backgroundColor = kHooDatePickerScrolViewBackgroundColor;
    _scrollViewWeeks.showsHorizontalScrollIndicator = NO;
    _scrollViewWeeks.showsVerticalScrollIndicator = NO;
    [self addSubview:_scrollViewWeeks];
    
    _lineWeeksTop = [[UIView alloc] initWithFrame:CGRectMake(_scrollViewWeeks.frame.origin.x + kHooDatePickerLineMargin, _scrollViewWeeks.frame.origin.y + (_scrollViewWeeks.frame.size.height / 2) - (kHooDatePickerScrollViewItemHeight / 2), width - (2 * kHooDatePickerLineMargin), kHooDatePickerLineWidth)];
    _lineWeeksTop.backgroundColor = self.highlightColor;
    [self addSubview:_lineWeeksTop];
    
    _lineWeeksBottom = [[UIView alloc] initWithFrame:CGRectMake(_scrollViewWeeks.frame.origin.x + kHooDatePickerLineMargin, _scrollViewWeeks.frame.origin.y + (_scrollViewWeeks.frame.size.height / 2) + (kHooDatePickerScrollViewItemHeight / 2), width - (2 * kHooDatePickerLineMargin), kHooDatePickerLineWidth)];
    _lineWeeksBottom.backgroundColor = self.highlightColor;
    [self addSubview:_lineWeeksBottom];
    
    // Update ScrollView Data
    [self buildSelectorLabelsWeekNames];
    
    UITapGestureRecognizer *singleTap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(singleTapGestureWeekNameCaptured:)];
    singleTap.numberOfTapsRequired = 1;
    singleTap.numberOfTouchesRequired = 1;
    [_scrollViewWeeks addGestureRecognizer:singleTap];
    
}


- (void)buildSelectorLabelsWeekNames {
    CGFloat offsetContentScrollView = (_scrollViewWeeks.frame.size.height - kHooDatePickerScrollViewItemHeight) / 2.0;
    
    if (_labelWeeks && _labelWeeks.count > 0) {
        for (UILabel *label in _labelWeeks) {
            [label removeFromSuperview];
        }
    }
    
    _labelWeeks = [[NSMutableArray alloc] init];
    
    for (int i = 0; i < _weeks.count; i++) {
        
        NSString *name = (NSString*)[_weeks objectAtIndex:i];
        
        UILabel *labelDay = [[UILabel alloc] initWithFrame:CGRectMake(0.0, (i * kHooDatePickerScrollViewItemHeight) + offsetContentScrollView, _scrollViewWeeks.frame.size.width, kHooDatePickerScrollViewItemHeight)];
        labelDay.text = name;
        labelDay.font = kHooDatePickerLabelFont;
        labelDay.textAlignment = NSTextAlignmentCenter;
        labelDay.textColor = self.tintColor;
        ...
        [_labelWeeks addObject:labelDay];
        [_scrollViewWeeks addSubview:labelDay];
    }
    
    _scrollViewWeeks.contentSize = CGSizeMake(_scrollViewWeeks.frame.size.width, (kHooDatePickerScrollViewItemHeight * _weeks.count) + (offsetContentScrollView * 2));
}

- (void)removeSelectorWeeks {
    if (_scrollViewWeeks) {
        [_scrollViewWeeks removeFromSuperview];
        _scrollViewWeeks = nil;
    }
    if (_lineWeeksTop) {
        [_lineWeeksTop removeFromSuperview];
        _lineWeeksTop = nil;
    }
    if (_lineWeeksBottom) {
        [_lineWeeksBottom removeFromSuperview];
        _lineWeeksBottom = nil;
    }
}
...
- (void)actionButtonValid:(UIButton *)sender {
    ...
    [self dismiss];
    if (_datePickerMode == 6) {
        if ([self.delegate respondsToSelector:@selector(datePicker:didSelectedWeek:)]) {
            [self.delegate datePicker:self didSelectedWeek:_selectdWeek];
        }
    }else {
...
- (void)actionButtonDateModel:(UIButton *)sender {
    
    
    if ([self.delegate respondsToSelector:@selector(datePicker:didSelectedDateModel:)]) {
        [self.delegate datePicker:self didSelectedDateModel:[self getDate]];
    }
}

- (void)actionButtonTimeModel:(UIButton *)sender {
    
    if ([self.delegate respondsToSelector:@selector(datePicker:didSelectedTimeModel:)]) {
        [self.delegate datePicker:self didSelectedTimeModel:[self getDate]];
    }
}

- (void)setHighLightModel:(int)index {
    if (index == 0) {
        [dateModel.layer setMasksToBounds:YES];
        [dateModel.layer setCornerRadius:4.0];
        [dateModel.layer setBorderWidth:1.0];//边框宽度
        UIColor *bgColor = [UIColor colorWithRed:51/255.0f green:181/255.0f blue:229/255.0f alpha:1.0f];
        [dateModel.layer setBorderColor:bgColor.CGColor];
        [dateModel setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        [dateModel setBackgroundColor:bgColor];
        
        [timeModel.layer setMasksToBounds:YES];
        [timeModel.layer setCornerRadius:4.0];
        [timeModel.layer setBorderWidth:1.0];//边框宽度
        [timeModel.layer setBorderColor:[UIColor blackColor].CGColor];
        [timeModel setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
        [timeModel setBackgroundColor:[UIColor whiteColor]];
    }else{
        [dateModel.layer setMasksToBounds:YES];
        [dateModel.layer setCornerRadius:4.0];
        [dateModel.layer setBorderWidth:1.0];//边框宽度
        [dateModel.layer setBorderColor:[UIColor blackColor].CGColor];
        [dateModel setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
        [dateModel setBackgroundColor:[UIColor whiteColor]];
        
        UIColor *bgColor = [UIColor colorWithRed:51/255.0f green:181/255.0f blue:229/255.0f alpha:1.0f];
        [timeModel.layer setMasksToBounds:YES];
        [timeModel.layer setCornerRadius:4.0];
        [timeModel.layer setBorderWidth:1.0];//边框宽度
        [timeModel.layer setBorderColor:bgColor.CGColor];
        [timeModel setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        [timeModel setBackgroundColor:bgColor];
    }
}
...
-(void)show {
    ...
    if (self.datePickerMode == HooDatePickerModeWeek) {
        int indexWeekName = [self getIndexForScrollViewPosition:_scrollViewWeeks];
        [self highlightLabelInArray:_labelWeeks atIndex:indexWeekName];
    }
...
- (void)clickBackView {
    if (!_superView) return;
    if ([self.delegate respondsToSelector:@selector(datePicker:dismiss:)]) {
        [self.delegate datePicker:self dismiss:_datePickerMode];
    }
}
...
- (void)singleTapGestureWeekCaptured:(UITapGestureRecognizer *)gesture {
    
    CGPoint touchPoint = [gesture locationInView:self];
    CGFloat touchY = touchPoint.y;
    
    if (touchY < (_lineWeeksTop.frame.origin.y)) {
        
        if (_selectdWeek > 0) {
            _selectdWeek -= 1;
            [self setScrollView:_scrollViewWeeks atIndex:_selectdWeek animated:YES];
        }
        
    } else if (touchY > (_lineWeeksBottom.frame.origin.y)) {
        
        if (_selectdWeek < _weeks.count - 1) {
            _selectdWeek += 1;
            [self setScrollView:_scrollViewWeeks atIndex:_selectdWeek animated:YES];
        }
    }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    }else if (scrollView.tag == ScrollViewTagValue_WEEKS) {
        [self highlightLabelInArray:_labelWeeks atIndex:index];
    }
}
...
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate {
    ...
    // 周
    if (self.datePickerMode == HooDatePickerModeWeek) {
        if (_selectdWeek > _maxWeekIndex) {
            [self setWeek:_maxWeekIndex animated:YES];
        }
    }
...
- (void)setDate:(NSDate *)date animated:(BOOL)animated {
    ...
    if (self.delegate != nil && [self.delegate respondsToSelector:@selector(datePicker:dataDidChange:)]) {
        [self.delegate datePicker:self dataDidChange:[self getWeek]];
    }
}

#pragma mark - Week
- (void)setWeek:(NSInteger)weekIndex andWeekName:(NSArray *)weekNames andMaxWeek:(NSInteger)weekMaxIndex animated:(BOOL)animated {
    if (weekNames && weekIndex >= 0) {
        self.weeks = [NSMutableArray arrayWithArray:weekNames];
        _selectdWeek = weekIndex;
        _maxWeekIndex = weekMaxIndex;
        [self setupControl];
        [self setScrollView:_scrollViewWeeks atIndex:(_selectdWeek) animated:animated];
        if (self.delegate != nil && [self.delegate respondsToSelector:@selector(datePicker:dataDidChange:)]) {
            [self.delegate datePicker:self dataDidChange:[self getWeek]];
        }
    }
...
- (void)setWeek:(NSInteger)weekIndex animated:(BOOL)animated {
    _selectdWeek = weekIndex;
    [self setScrollView:_scrollViewWeeks atIndex:(_selectdWeek) animated:animated];
    if (self.delegate != nil && [self.delegate respondsToSelector:@selector(datePicker:dataDidChange:)]) {
        [self.delegate datePicker:self dataDidChange:[self getWeek]];
...
- (NSMutableDictionary*)getWeek {
    if (self.datePickerMode == HooDatePickerModeWeek) {
        NSMutableDictionary *result = [NSMutableDictionary dictionary];
        [result setObject:@(_selectdWeek) forKey:@"weekIndex"];
        [result setObject:[_weeks objectAtIndex:_selectdWeek] forKey:@"weekName"];
        return result;
    }
    return nil;
}
...
- (UIView *)headerView {
    if (!_headerView) {
        ...
        [_cancelButton setTitleColor:KHOODataPickerCancelTxtColor forState:UIControlStateNormal];
        ...
        [_sureButton setTitleColor:KHOODataPickerSureTxtColor forState:UIControlStateNormal];
        ...
        if (initDateType == 5) {
            // 模式
            dateModel = [[UIButton alloc] initWithFrame:CGRectMake(self.frame.size.width/2 - kHooDatePickerButtonHeaderWidth -10, kHooDatePickerHeaderHeight/4, kHooDatePickerButtonHeaderWidth, kHooDatePickerHeaderHeight/2)];
            timeModel = [[UIButton alloc] initWithFrame:CGRectMake(self.frame.size.width/2 + 10, kHooDatePickerButtonHeaderWidth/4, kHooDatePickerButtonHeaderWidth, kHooDatePickerHeaderHeight/2)];
            UIColor *bgColor = [UIColor colorWithRed:51/255.0f green:181/255.0f blue:229/255.0f alpha:1.0f];
            [dateModel.layer setMasksToBounds:YES];
            [dateModel.layer setCornerRadius:4.0];
            [dateModel.layer setBorderWidth:1.0];//边框宽度
            ...
            [dateModel setTitle:@"日期" forState:UIControlStateNormal];
            [dateModel setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
            [dateModel setBackgroundColor:bgColor];
            [dateModel addTarget:self action:@selector(actionButtonDateModel:) forControlEvents:UIControlEventTouchUpInside];
            [_headerView addSubview:dateModel];
            
            [timeModel.layer setMasksToBounds:YES];
            [timeModel.layer setCornerRadius:4.0];
            [timeModel.layer setBorderWidth:1.0];//边框宽度
            [timeModel setTitle:@"时间" forState:UIControlStateNormal];
            [timeModel setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
            [timeModel setBackgroundColor:[UIColor whiteColor]];
            
            [timeModel addTarget:self action:@selector(actionButtonTimeModel:) forControlEvents:UIControlEventTouchUpInside];
            [_headerView addSubview:timeModel];
        }
    }
    return _headerView;
}
...
- (NSMutableArray*)minutes {
    if (!_minutes) {
        ...
        for (int i = 0; i < 60; i++) {
            ...
            if (i<10) {
                [minutes addObject:[NSString stringWithFormat:@"0%d%@", i, @"分"]];
            } else {
...
- (void)setKSureButtonText:(NSString *)sureTxt{
    [_sureButton setTitle:sureTxt forState:UIControlStateNormal];
}

- (void)setKCancelButtonText:(NSString *)cancelTxt {
    [_cancelButton setTitle:cancelTxt forState:UIControlStateNormal];
}

- (void)setKSureButtonTextColor:(UIColor *)sureTxtColor {
    [_sureButton setTitleColor:sureTxtColor forState:UIControlStateNormal];
}

- (void)setKCancelButtonTextColor:(UIColor *)cancelTxtColor {
    [_cancelButton setTitleColor:cancelTxtColor forState:UIControlStateNormal];
}
