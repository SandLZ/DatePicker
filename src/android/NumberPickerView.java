...
/**
 * Created by liuzhu on 2017/1/16.
 * Description :
 * Usage :
 */
public class NumberPickerView extends View {
    ...
    public NumberPickerView(Context context, AttributeSet attrs) {
        ...
        initAttr2(context, attrs,0);
        init(context);
    }
    public NumberPickerView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        fakeR = new FakeR(context);
        initAttr2(context, attrs,defStyleAttr);
        init(context);
    }

    private void initAttr2(Context context, AttributeSet attrs, int defStyleAttr) {
        //
        // 行
        int attrId = context.getResources().getIdentifier("npv_ShowCount", "attr", context.getPackageName());
        final TypedArray a = context.obtainStyledAttributes(attrs, new int[]{attrId}, defStyleAttr, 0);
        mShowCount = a.getInt(fakeR.getId("styleable","NumberPickerView_npv_ShowCount"), 3);
        // 分割线颜色
        int attrId2 = context.getResources().getIdentifier("npv_DividerColor", "attr", context.getPackageName());
        final TypedArray b = context.obtainStyledAttributes(attrs, new int[]{attrId2}, defStyleAttr, 0);
        mDividerColor = b.getColor(fakeR.getId("styleable","NumberPickerView_npv_DividerColor"), DEFAULT_DIVIDER_COLOR);
        // npv_TextSizeNormal
        int attrId3 = context.getResources().getIdentifier("npv_TextSizeNormal", "attr", context.getPackageName());
        final TypedArray c = context.obtainStyledAttributes(attrs, new int[]{attrId3}, defStyleAttr, 0);
        mTextSizeNormal = (int) c.getDimension(fakeR.getId("styleable","NumberPickerView_npv_TextSizeNormal"), sp2px(context, DEFAULT_TEXT_SIZE_NORMAL_SP));
        // npv_HintText
        int attrId4 = context.getResources().getIdentifier("npv_HintText", "attr", context.getPackageName());
        final TypedArray d = context.obtainStyledAttributes(attrs, new int[]{attrId4}, defStyleAttr, 0);
        mHintText = d.getString(fakeR.getId("styleable","NumberPickerView_npv_HintText"));
        // npv_TextColorHint
        int attrId5 = context.getResources().getIdentifier("npv_TextColorHint", "attr", context.getPackageName());
        final TypedArray e = context.obtainStyledAttributes(attrs, new int[]{attrId5}, defStyleAttr, 0);
        mTextColorHint = e.getColor(fakeR.getId("styleable","NumberPickerView_npv_TextColorHint"), DEFAULT_TEXT_COLOR_SELECTED);
        // npv_TextArray
        int attrId6 = context.getResources().getIdentifier("npv_TextArray", "attr", context.getPackageName());
        final TypedArray f = context.obtainStyledAttributes(attrs, new int[]{attrId6}, defStyleAttr, 0);
        mDisplayedValues = convertCharSequenceArrayToStringArray(f.getTextArray(fakeR.getId("styleable","NumberPickerView_npv_TextArray")));
        // week
        int attrId10 = context.getResources().getIdentifier("npv_Week", "attr", context.getPackageName());
        final TypedArray j = context.obtainStyledAttributes(attrs, new int[]{attrId10}, defStyleAttr, 0);
        week = j.getBoolean(fakeR.getId("styleable","NumberPickerView_npv_Week"),false);
        if (week) {
            // 设置week的值
        }
        // npv_TextSizeSelected
        int attrId7 = context.getResources().getIdentifier("npv_TextSizeSelected", "attr", context.getPackageName());
        final TypedArray g = context.obtainStyledAttributes(attrs, new int[]{attrId7}, defStyleAttr, 0);
        mTextSizeSelected = (int) g.getDimension(fakeR.getId("styleable","NumberPickerView_npv_TextSizeSelected"), sp2px(context, DEFAULT_TEXT_SIZE_SELECTED_SP));
        // npv_RespondChangeOnDetached
        int attrId8 = context.getResources().getIdentifier("npv_RespondChangeOnDetached", "attr", context.getPackageName());
        final TypedArray h = context.obtainStyledAttributes(attrs, new int[]{attrId8}, defStyleAttr, 0);
        mRespondChangeOnDetach = h.getBoolean(fakeR.getId("styleable","NumberPickerView_npv_RespondChangeOnDetached"), false);
        // npv_WrapSelectorWheel
        int attrId9 = context.getResources().getIdentifier("npv_WrapSelectorWheel", "attr", context.getPackageName());
        final TypedArray i = context.obtainStyledAttributes(attrs, new int[]{attrId9}, defStyleAttr, 0);
        mWrapSelectorWheel = i.getBoolean(fakeR.getId("styleable","NumberPickerView_npv_WrapSelectorWheel"), false);
        a.recycle();
        b.recycle();
        c.recycle();
        d.recycle();
...
//    }
    ...
    public void setDisplayedValuesOfWeek(String[] displayedValues) {
